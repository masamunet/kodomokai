  ...
  return (
    <Box className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <Heading size="h1" className="text-3xl font-bold tracking-tight text-foreground">会計報告</Heading>
          <Text className="text-muted-foreground text-sm">決算報告書および予算案の管理</Text>
        </div>
        <Link
      ...
      <Box className="grid grid-cols-1 gap-4">
        {reports.length === 0 ? (
          <Box className="bg-card border border-border p-16 rounded-xl text-center shadow-sm">
            <Text className="text-muted-foreground">会計報告がまだありません。「新規作成」から始めましょう。</Text>
          </Box>
        ) : (
          reports.map((report) => (
            <Box key={report.id} className="bg-card border border-border p-5 rounded-xl flex items-center justify-between group hover:border-primary/30 hover:shadow-md transition-all">
              <Box className="space-y-2">
                ...
                <Heading size="h3" className="text-lg font-bold text-foreground">{report.title}</Heading>
                <Text className="text-xs text-muted-foreground">最終更新: {new Date(report.updated_at).toLocaleDateString()}</Text>
              </Box>
