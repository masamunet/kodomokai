...
export function UserImportScreen() {
  ...
  return (
    <Stack className="gap-6">
      <Box className="flex justify-between items-center">
        <Heading size="h2" className="text-xl font-bold">データ一括登録</Heading>
      </Box>
      ...
      <Box className="bg-white p-6 rounded-lg shadow space-y-4">
        <Box className="bg-blue-50 p-4 rounded text-sm text-blue-800">
          <Text weight="bold" className="block mb-2">
          ...
          <Box className="mt-2">
            <Text weight="bold" className="text-xs text-red-600">
      ...
      {result && (
        <Box className="bg-white p-6 rounded-lg shadow space-y-4">
          <Box className="flex justify-between items-center border-b pb-2">
            <Heading size="h3" className="font-bold">実行結果</Heading>
            {!result.success && (
          ...
          <Box className="flex gap-4">
            <Box className={`flex items-center gap-2 ${result.success ? 'text-green-600' : 'text-gray-400'}`}>
              <CheckCircle size={20} />
              <span className="font-bold">成功: {result.createdCount}件</span>
            </Box>
            <Box className={`flex items-center gap-2 ${result.success ? 'text-gray-400' : 'text-red-600'}`}>
              <AlertCircle size={20} />
          ...
          {result.gradeSummary && Object.keys(result.gradeSummary).length > 0 && (
            <Box className="bg-indigo-50 p-6 rounded-lg border border-indigo-100 space-y-3">
              <Heading size="h4" className="font-bold text-indigo-900 flex items-center gap-2">
                <Baby size={18} />
                登録お子様の学年別内訳
              </Heading>
              <Box className="bg-white rounded border border-indigo-200 overflow-hidden">
          ...
          <Box className={`bg-gray-50 p-4 rounded border h-64 overflow-y-auto font-mono text-xs ${!result.success ? 'border-red-200' : ''}`}>
